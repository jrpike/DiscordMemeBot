...
async def on_message(message):
	try:
		...
		if content == "-roll":
			...
			asyncio.get_event_loop().create_task(curr_channel.send(file=discord.File(local_filename)))
			...
			if template not in templates:
				asyncio.get_event_loop().create_task(message.add_reaction(random.choice(Attribs.bad_reacts)))
				...
				while error:
					try:
						...
						asyncio.get_event_loop().create_task(curr_channel.send(template + " seems corrupt"))
				...
				if not error:
					...
					asyncio.get_event_loop().create_task(curr_channel.send(file=discord.File(filename)))
					os.system("rm -f \"null\"")
					clean_image(filename)
			...
			asyncio.get_event_loop().create_task(curr_channel.send(template_list_str))
			...
			asyncio.get_event_loop().create_task(curr_channel.send(Attribs.audio_meme_list_str))
